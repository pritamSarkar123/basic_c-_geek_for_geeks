...
int _merge(vector<int> &arr,int low,int mid,int high,vector<int> &temp){
    int i,j,m,n,k;
...
int mergeSort(vector<int> &arr,int low,int high,vector<int> &temp){
    ...
    if(high > low){
        ...
        invert_count+=_merge(arr,low,mid,high,temp);
    }
    return invert_count;
}

int main(){
    vector<int> arr{10,9,8,7,6,5,12,4,3,12,2,2,1,20,10,9,8,7,6,5,12,4,3,12,2,2,1,20};
