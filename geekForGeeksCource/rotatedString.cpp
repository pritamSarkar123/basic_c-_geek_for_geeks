...
using namespace std;
void foo(){
    //Problem description:- https://www.youtube.com/watch?v=8rfrm1RKp5M&feature=emb_logo
    string A="ABCD";
    string B="DABC";
...
void foo2(){
    //Problem description:- https://www.youtube.com/watch?v=8sXLOIBFLGk&feature=emb_logo
    string str="Pritam likes doing coding";
    vector<string> vstr;
    int i=0;
    int len=0;
    int start=0;
    while(i<str.length()){
        if(str[i]==' '){
            vstr.push_back(str.substr(start,len));
            len=0;start=i+1;
        }else{
            len++;
        }
...
void regularExpressionBasic(){
    //Problem description:- https://www.youtube.com/watch?v=u-bFUwdPMpA&feature=emb_logo
    string baseString="hello world I10 I am pritam Y59 sarkar how are u V78?";
    regex regularExpression("[A-Z][0-9][0-9]");
    smatch matched; //matching string container
    while(regex_search(baseString,matched,regularExpression)){
        cout<<"matching :- "<<matched.str()<<endl;
...
void firstIO(){
	...
	//this line turn off the synchronization between c++ and c standard stream
	// to avoid overriding
